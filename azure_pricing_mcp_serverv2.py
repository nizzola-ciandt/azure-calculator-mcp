...
class HealthCheckEndpoint(HTTPEndpoint):
    ...
    Returns application health status and basic diagnostic information.
    """
    async def get(self, request):
        try:
            # Test connectivity with Azure Pricing API
            test_url = "https://prices.azure.com/api/retail/prices"
            test_params = {
            ...
            log(f"Health check API test failed: {str(e)}", "warning")
            api_healthy = False
        
        health_status = {
            "status": "healthy" if api_healthy else "degraded",
            "timestamp": datetime.utcnow().isoformat() + "Z",
            "service": "Azure Pricing MCP Server",
            "version": "1.0.0",
            "checks": {
                "application": "ok",
                "azure_pricing_api": "ok" if api_healthy else "unavailable"
            },
            "endpoints": {
                "sse": "/sse",
                "tools": "/tools",
        ...
        log(f"Health check performed - Status: {health_status['status']}")
...
class RootEndpoint(HTTPEndpoint):
    ...
    async def get(self, request):
        info = {
            "service": "Azure Pricing MCP Server",
            "version": "1.0.0",
            "description": "Model Context Protocol server for Azure pricing information",
            "endpoints": {
            ...
            "documentation": "https://github.com/nizzola-ciandt/azure-calculator-mcp",
            "status": "running"
        }
